...
export class Line2D {
  ...
  public constructor(a: Point2D, b: Point2D) {
    if (a.equals(b))
      throw new Error(`Lines are defined by two distinct points.`);
  ...
  public static from(
    form: "SLOPE_INTERCEPT" | "POINT_SLOPE" | "STANDARD",
    a: number | Point2D,
    b: number,
    c?: number
  ) {
    if (form === "SLOPE_INTERCEPT")
      return new Line2D(new Point2D(0, b), new Point2D(1, b - (a as number)));

    if (form === "POINT_SLOPE")
      return new Line2D(
        a as Point2D,
        new Point2D((a as Point2D).x + 1, (a as Point2D).y - b)
      );

    if (form === "STANDARD")
      return Line2D.from("SLOPE_INTERCEPT", -a / b, (c as number) / b);
